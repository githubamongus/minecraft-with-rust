...
impl Chunk {
    pub fn new(block_list: &HashMap<Box<str>, Block>) -> Chunk {
        let mut blocks = vec![vec![vec![block_list.get("air").unwrap().clone(); 10]; 256]; 10];

        for x in 0..10 {
            for y in 0..256 {
                for z in 0..10 {
                    if y <= 9 {
                        blocks[x][y][z] = block_list.get("grass_block").unwrap().clone();
                    }
                }
            }
        }
